...
bool FunctionDesc::isMin() {
	return expression.compare(FUNC_MIN) == 0;
}

bool FunctionDesc::isMax() {
	return expression.compare(FUNC_MAX) == 0;
}

bool FunctionDesc::isSum() {
	return expression.compare(FUNC_SUM) == 0;
}

bool FunctionDesc::isCount() {
	return expression.compare(FUNC_COUNT) == 0;
}

std::string FunctionDesc::getExpression() {
	return expression;
}

void FunctionDesc::setExpression(const std::string & expression) {
	this->expression = expression;
}
ParameterDesc * FunctionDesc::getParameter() {
	return parameter;
}

void FunctionDesc::setParameter(ParameterDesc * parameter) {
	this->parameter = parameter;
}

std::string FunctionDesc::getReturnType() {
	return returnType;
}
...
DataType * FunctionDesc::getReturnDataType() {
